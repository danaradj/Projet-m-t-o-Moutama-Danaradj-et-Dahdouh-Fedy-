...
Chainon *add_word_liste (Chainon **pliste) {
  FILE *fichier = NULL;
  char str[] = "donnees-synop-essentielles-omm.csv"; //on ouvre le fichier csv qui contient les données méteo
  fichier = fopen(str, "r"); 
  if(fichier == NULL) {
...
FILE *Ecrire_donnee_trier(char *str, Chainon *pliste) {
  ...
  FILE* fichier = NULL;
  fichier = fopen(str, "w"); //On ouvre le fichier de sortie qui va contenir les données triées
  while (pliste != NULL) {
